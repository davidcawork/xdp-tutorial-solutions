...
static void stats_poll(int map_fd, __u32 map_type, int interval, const char * dir, __u32 id)
{
	...
	while (1) {
		...
		// Lets OPEN each time to check if id params is different
		map_fd = open_bpf_map_file(dir, "xdp_stats_map", &info);
		if (map_fd < 0) {
