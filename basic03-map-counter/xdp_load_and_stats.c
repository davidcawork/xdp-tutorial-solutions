			...
			struct stats_record *stats_prev)
{
	...
	for ( i = 0 ; i < XDP_ACTION_MAX; i++)
	{
		...
		data	= rec->total.byte_counter - prev->total.byte_counter;
		mbps	= (data * 8)/period / 1000000;

		printf(fmt, action, rec->total.rx_packets, pps, rec->total.byte_counter / 1000, mbps, period);
	}
	printf("\n\n");
}
...
void map_get_value_percpu_array(int fd, __u32 key, struct datarec *value)
{
	...
	for (i = 0; i < nr_cpus; i++) {
		...
		sum_bytes += values[i].byte_counter;
	}
	value->rx_packets = sum_pkts;
	value->byte_counter   = sum_bytes;
}
...
static bool map_collect(int fd, __u32 map_type, __u32 key, struct record *rec)
{
	...
	rec->total.byte_counter = value.byte_counter;
